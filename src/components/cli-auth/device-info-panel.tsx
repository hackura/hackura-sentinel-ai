...
import { ShieldAlert, Clock, Terminal, AlertCircle } from 'lucide-react';
    ...
    >
      ...
      {isValid ? (
        <div className="grid gap-3 sm:grid-cols-[1.2fr_0.8fr] sm:items-start">
          <div>
            ...
            <p className="mt-2 text-xs text-zinc-400">
              This device ID was securely passed from your CLI. Each authorization is logged.
            </p>
          </div>
        ...
        <div className="rounded-xl border border-red-500/20 bg-red-500/10 p-3 text-sm text-red-100">
          ...
          <p className="text-xs">
            The device ID is missing or invalid. This session cannot be authorized. Please restart your CLI.
          </p>
        </div>
      )}
      ...
      <div className="mt-4 flex flex-wrap gap-2 text-xs text-zinc-400">
        ...
        <span className="rounded-full border border-white/8 bg-white/5 px-3 py-1">
          JWT stored securely in backend
        </span>
      </div>
    </motion.div>
  );
